...
const createUserSchema = Joi.object({
    firstName: Joi.string().min(2).max(50).pattern(/^[A-Za-z\sáéíóúÁÉÍÓÚüÜ]+$/u).required().messages({
    ...
    lastName: Joi.string().min(2).max(50).pattern(/^[A-Za-z\sáéíóúÁÉÍÓÚüÜ]+$/u).required().messages({
        'string.empty': 'El apllido no puede estar vacío',
